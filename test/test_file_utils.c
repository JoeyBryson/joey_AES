#include "../file-utilities/utils.h"
...
{
    const char* in_file_path = "test/test-files/in-files/test_text1.txt";
    const char* out_dir = "test/test-files/out-files";
    ...
    free(byte_arr.bytes);
    fclose(out_file_ptr);
    TEST_ASSERT_EQUAL_INT32_MESSAGE(1, files_are_equal(in_file_path, "test/test-files/out-files/test_text1.txt"),  "in file and out files different");
}

void test_plain_file_to_byte_arr_to_state_arr_to_byte_arr_to_file_again()
{
    const char* in_file_path = "test/test-files/in-files/test_text2.txt";
    const char* out_dir = "test/test-files/out-files";
    ...
    fclose(out_file_ptr);
    TEST_ASSERT_EQUAL_INT32_MESSAGE(1, files_are_equal(in_file_path, "test/test-files/out-files/test_text2.txt"),  "failed to copy contents correctly");
}

void test_encrypt_file_creates_a_file()
{
    char* plaintext_path = "test/test-files/in-files/test_text3.txt";
    char* cipher_dir = "test/test-files/cipher-files";
    ...
    TEST_ASSERT_EQUAL_INT32_MESSAGE(1, file_exists("test/test-files/cipher-files/test_3_cipher.joey"),  "failed to generate file");
    free(aes_key.words);
}

void test_cipher_file_to_states_to_file_again()
{
    const char* in_path = "test/test-files/cipher-files/test_3_cipher.joey";
    const char* cipher_dir = "test/test-files/cipher-files-copied";
    ...
    TEST_ASSERT_EQUAL_INT32_MESSAGE(1, files_are_equal("test/test-files/cipher-files/test_3_cipher.joey", "test/test-files/cipher-files-copied/test_3_cipher.joey"),  "failed to copy contents correctly");
    free(state_arr.states);
}

void test_decrypt_previously_encrypted_file()
{
    const char* plain_out_dir = "test/test-files/out-files";
    const char* cipher_path = "test/test-files/cipher-files/test_3_cipher.joey";
    aes_key_t aes_key;
    aes_key.num_key_words = 4; 
    aes_key.alg = AES128;
    aes_key.words = malloc(sizeof(word_t) * aes_key.num_key_words);
    ...
    TEST_ASSERT_EQUAL_INT32_MESSAGE(1, files_are_equal("test/test-files/out-files/test_text3.txt", "test/test-files/in-files/test_text3.txt"), "decrypted file does not match original");
    free(aes_key.words);
}

void test_encrypt_decrypt_large_jpeg()
{
    const char* plaintext_path = "test/test-files/in-files/wales.jpg";
    const char* cipher_dir = "test/test-files/cipher-files";
    const char* cipher_name = "big1";
    const char* plain_out_dir = "test/test-files/out-files";
    const char* expected_cipher_path = "test/test-files/cipher-files/big1.joey";
    aes_key_t aes_key;
    aes_key.num_key_words = 4;
    aes_key.alg = AES128;
    aes_key.words = malloc(sizeof(word_t) * aes_key.num_key_words);
    ...
    TEST_ASSERT_EQUAL_INT32_MESSAGE(1, files_are_equal("test/test-files/in-files/wales.jpg", "test/test-files/out-files/wales.jpg"), "decrypted file does not match original");
    clock_t _end = clock();
    ...
    if (stat("test/test-files/out-files/wales.jpg", &_st_dec) == 0) printf("wales.jpg decrypted file size: %lld bytes\n", (long long)_st_dec.st_size);
    printf("test_encrypt_decrypt_large_jpeg elapsed time: %.3f seconds\n", _elapsed);
}

void test_encrypt_decrypt_large_png()
{
    const char* plaintext_path = "test/test-files/in-files/attractor.png";
    const char* cipher_dir = "test/test-files/cipher-files";
    const char* cipher_name = "big2";
    const char* plain_out_dir = "test/test-files/out-files";
    const char* expected_cipher_path = "test/test-files/cipher-files/big2.joey";
    aes_key_t aes_key;
    aes_key.num_key_words = 4;
    aes_key.alg = AES128;
    aes_key.words = malloc(sizeof(word_t) * aes_key.num_key_words);
