...
// inverse of test used in 128 bit test in test_cipher.c
void test_AES128_inv_cipher_should_recover_plaintext(void)
{
	aes_key_t aes_key;
	...
	state_t ciphertext = {.col[0] = (word_t){{0x39, 0x25, 0x84, 0x1D}},
	                      .col[1] = (word_t){{0x02, 0xDC, 0x09, 0xFB}},
	                      .col[2] = (word_t){{0xDC, 0x11, 0x85, 0x97}},
	                      .col[3] = (word_t){{0x19, 0x6A, 0x0B, 0x32}}};

	state_t expected_plaintext = {.col[0] = (word_t){{0x32, 0x43, 0xF6, 0xA8}},
	...
	state_t result = inv_cipher(round_keys, ciphertext);

	TEST_ASSERT_TRUE_MESSAGE(states_equal(&result, &expected_plaintext),
	                         "AES-128 decryption output does not match expected plaintext");

	free(aes_key.words);
	free(round_keys.words);
}

// test vector taken from first line of plaintext on page 12 of the NIST Block Cipher Modes of
// Operation Electronic Codebook (ECB) inverse of test used in 192 bit test in test_cipher.c
// https://csrc.nist.gov/CSRC/media/Projects/Cryptographic-Standards-and-Guidelines/documents/examples/AES_Core_All.pdf
void test_AES192_inv_cipher_should_recover_plaintext(void)
{
	aes_key_t aes_key;
	...
	state_t ciphertext = {.col[0] = (word_t){{0xBD, 0x33, 0x4F, 0x1D}},
	                      .col[1] = (word_t){{0x6E, 0x45, 0xF2, 0x5F}},
	                      .col[2] = (word_t){{0xF7, 0x12, 0xA2, 0x14}},
	                      .col[3] = (word_t){{0x57, 0x1F, 0xA5, 0xCC}}};

	state_t expected_plaintext = {.col[0] = (word_t){{0x6B, 0xC1, 0xBE, 0xE2}},
	...
	TEST_ASSERT_TRUE_MESSAGE(states_equal(&result, &expected_plaintext),
	                         "AES-192 decryption output does not match expected plaintext");

	free(aes_key.words);
	free(round_keys.words);
}

// test vectors is first line of plaintext on page 25 of the NIST Block Cipher Modes of Operation
// Electronic Codebook (ECB) inverse of test used in 256 bit test in test_cipher.c
// https://csrc.nist.gov/CSRC/media/Projects/Cryptographic-Standards-and-Guidelines/documents/examples/AES_Core_All.pdf
void test_AES256_inv_cipher_should_recover_plaintext(void)
{
	aes_key_t aes_key;
	...
	state_t ciphertext = {.col[0] = (word_t){{0xF3, 0xEE, 0xD1, 0xBD}},
	...
	TEST_ASSERT_TRUE_MESSAGE(states_equal(&result, &expected_plaintext),
	                         "AES-256 decryption output does not match expected plaintext");
...
int main(void)
{
	...
	RUN_TEST(test_AES128_inv_cipher_should_recover_plaintext);
	RUN_TEST(test_AES192_inv_cipher_should_recover_plaintext);
	RUN_TEST(test_AES256_inv_cipher_should_recover_plaintext);
