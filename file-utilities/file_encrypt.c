...
//magic numbers for ciper .joey files
...
//sets byte_arr to contain all the bytes to be encrypted: the header(name length & name) and file contents itself
byte_arr_t plain_file_to_byte_arr(FILE* file_ptr, const char* plain_file_name) 
{
    ...
    if (fread(byte_arr.bytes + 1 + name_len, sizeof(byte_t), file_size, file_ptr) != (size_t)file_size) {
...
//creating a .joey file consisting of magic numbers, cipher name, initialization vector(plain) and cipher-bytes respectively
...
state_t gen_iv(void) 
{
    ...
    fclose(urandom_ptr);
    state_t injection_vector;
    buffer_to_state(&injection_vector, buffer);
...
FILE* build_cipher_file_ptr(const char* dir, const char* name)
{
    struct stat st;
    ...
    size_t path_len = strlen(dir) + strlen(name) + strlen(".joey") + 2; // 2 for '/' + '\0'
    char path[path_len];
    strcpy(path, dir);
    if (dir[strlen(dir) - 1] != '/') {
        strcat(path, "/");
...
void encrypt_file(const char* plain_file_path, const char* cipher_file_dir, const char* cipher_file_name, aes_key_t key) 
{
    ...
    FILE* cipher_ptr = build_cipher_file_ptr(cipher_file_dir, cipher_file_name);
